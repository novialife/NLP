...
class BigramTester(object):
            ...
            with codecs.open(filename, 'r', 'utf-8') as f:
                ...
                for row in f:
                    ...
                    if row[2].isdigit():
                        ...
                        self.unigram_count[row[1]] = float(row[2])/self.total_words
                    else:
                        self.bigram_prob[self.word[row[0]]][self.word[row[1]]] = math.exp(float(row[2]))
    ...
    def compute_entropy_cumulatively(self, word):
        try:
            ...
            try:
                big_prob = self.bigram_prob[prev_word][word]
            except KeyError:
                big_prob = 0
            ...
            entropy = self.lambda1 * float(big_prob) + self.lambda2 * float(self.unigram_count[word]) + self.lambda3

        except KeyError:
            entropy = self.lambda3

        self.logProb += math.log(entropy)/(-1*len(self.tokens))
        self.last_index += 1
...
def main():
    ...
    if arguments.check:
        ...
        requests.post(url=url, data=body, headers={'Connection': 'close'})
