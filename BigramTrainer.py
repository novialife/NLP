...
class BigramTrainer(object):
    ...
    def process_files(self, f):
        ...
        try:
            self.tokens = nltk.word_tokenize(
                text)  # Important that it is named self.tokens for the --check flag to work
        except LookupError:
            nltk.download('punkt')
    ...
    def stats(self):
        ...
        for key in self.unigram_count:
            rows_to_print.append("%i %s %i" % (self.index[key], key, self.unigram_count[key]))

        for dic in self.bigram_count:
            for key in self.bigram_count[dic]:
                cpair = self.bigram_count[dic][key]
                csingle = self.unigram_count[dic]
                prob = math.log(cpair / csingle)
                rows_to_print.append("%i %i %.15f" % (self.index[dic], self.index[key], prob))
