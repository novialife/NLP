...
class RandomIndexing(object):
    ...
    def clean_line(self, line):
        ...
        line = line.split(" ")
        return [x for x in line if x]  # Remove all empty strings
    ...
    def build_vocabulary(self):
        # YOUR CODE HERE
        for part in self.text_gen():  # Text gen returnerar en lista med sublistor för varje cleaned rad i filen
            for word in part:
    ...
    def create_word_vectors(self):
        ...
        for part in self.text_gen():  # Text gen returnerar en lista med sublistor för varje cleaned rad i filen
            for word in part:
                ...
                foo = np.zeros(d)
                indexes = random.sample(range(d), 100)
                for index in indexes:
                    foo[index] = 2 * np.random.randint(0, 2) - 1

                self.__rv[word] = foo
                self.__cv[word] = np.zeros(d)
        ...
        for i in range(len(word_list)):
            ...
            for j in range(1, right):
                if i + j >= len(word_list):
                    j = i + j - len(word_list)
                self.__cv[word_list[i]] += self.__rv[word_list[i + j]]

            for k in range(1, left):
                self.__cv[word_list[i]] += self.__rv[word_list[i - k]]
    ...
    def find_nearest(self, words, k=5, metric='cosine'):
        ...
        fitvec = np.array([self.__cv[i] for i in self.__cv.keys()])
        neigh.fit(fitvec)

        into = np.array([self.__cv[i] for i in words])
        out = neigh.kneighbors(into, n_neighbors=k)
        all_words = list(self.__cv.keys())
        output = []
        if len(words) > 1:
            for i in range(len(out[1])):
                sublist = []
                for j in range(len(out[1][1])):
                    sublist.append((all_words[out[1][i][j]], out[0][i][j]))
                output.append(sublist)
        else:
            sublist = []
            for j in range(len(out[1][0])):
                sublist.append((all_words[out[1][0][j]], out[0][0][j]))
        output.append(sublist)
        return output
